...
const GoogleLoginButton = () => {
  ...
  const t = useTranslations('GoogleLogin') // Load translations from 'GoogleLogin' namespace
      ...
      window.addEventListener('message', async (event) => {
        ...
        if (token) {
          ...
          if (isValid) {
            ...
            console.error(t('tokenValidationFailed'))
  ...
  const validateToken = async (token) => {
    try {
      ...
      console.error(t('tokenValidationError'), error.message)
      return false
    }
  }

  return <button onClick={handleLogin}>{t('loginButton')}</button>
}
