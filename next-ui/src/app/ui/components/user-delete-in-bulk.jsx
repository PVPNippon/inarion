...
/**
 * DeleteUsersViaCsvDialog is a component that provides a dialog for deleting users from
 * your domain via a CSV file.
 *
 * The component renders a dialog with a header, description, and form elements for
 * uploading a CSV file and deleting users. It also renders a table with the parsed
 * CSV data.
 *
 * The component handles errors and warnings for invalid file types, incorrect CSV
 * formatting, and deletion errors.
 *
 * The component is designed to be reusable in other contexts, and as such does not
 * include any specific styling or layout.
 */
function DeleteUsersViaCsvDialog() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`http://localhost:4000/api/users`, {
        headers: {
        ...
        data: {
          deleteUserEmails: userList,
        },
      })
      //if all users have been deleted successfully, the backend will send a 200 response. If deletion was successful partially, it will send a 207 response.
      //  Set the component state accordingly.
      setDeletionResult({ status: userStrings.deletionResultStatuses.success, responseData: response.data })
    } catch (err) {
      console.log('error', err)

      // error 400 means that the deletion was attempted, but failed for all users.
      if (err.response.status === 400 && err.response.data) {
        setDeletionResult({ status: userStrings.deletionResultStatuses.failure, responseData: err.response.data })
      } else {
        setDeletionResult({ status: userStrings.deletionResultStatuses.error, errorStatus: 500, errorDetails: err })
      }
    } finally {
      setTimeout(() => {
    ...
    <div>
      <Dialog onOpenChange={() => resetStates()}>
        <DialogTrigger asChild>
          <Button variant="default">Bulk delete users</Button>
        </DialogTrigger>
        ...
        >
          <div className="flex flex-col p-4 space-y-7">
            <DialogHeader>
              <DialogTitle className="font-semibold text-2xl/8">Bulk delete users</DialogTitle>
              <DialogDescription>You are about to delete multiple user(s) from your domain.</DialogDescription>
            </DialogHeader>
            ...
            >
                      ...
                      <div className="flex flex-col gap-y-1">
                        ...
                        <div className="text-muted-foreground text-sm/5">
                          {uploadState.status === userStrings.uploadStates.showTable
                            ...
                            : 'Upload a CSV file and review the list of users who will be removed.'}
                        </div>
                      </div>
              ...
              {uploadState.status === userStrings.uploadStates.showDeletionResult && (
                <div>
                  ...
                  {deletionResult && deletionResult.status === userStrings.deletionResultStatuses.error && (
                  ...
                  {deletionResult &&
                    (deletionResult.status === userStrings.deletionResultStatuses.success ||
                      deletionResult.status === userStrings.deletionResultStatuses.failure) && (
          ...
          {(uploadState.status === userStrings.uploadStates.showTable ||
            ...
            <DialogFooter
              className={
                uploadState.status === userStrings.uploadStates.deletionInProgress ||
                (uploadState.status === userStrings.uploadStates.showDeletionResult &&
                  deletionResult.status === userStrings.deletionResultStatuses.error)
                  ? 'md:justify-end'
                  : 'md:justify-between'
              }
            >
              ...
              {(uploadState.status === userStrings.uploadStates.showTable ||
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>{`Are you sure you want to delete ${csvData.length} user(s)?`}</AlertDialogTitle>
                      <AlertDialogDescription>
                        You can restore recently deleted users up to 20 days after deleting it from the Google Workspace
                        Admin Console. Learn more from Google's Help Center.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel className={userStyles.buttonPaddingWide}>Cancel</AlertDialogCancel>
                      <AlertDialogAction className={userStyles.buttonPaddingWide} onClick={handleDelete}>
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
...
export default DeleteUsersViaCsvDialog
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            ...
            <p>{`These headings help make sure that the right users are`}</p>
            <p>{`removed from your organization.`}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </div>
...
/**
 * A component that displays a loading spinner and a message indicating that the user removal process is in progress.
 *
 * The spinner is styled with a dashed border and a circular animation.
 *
 * @returns {JSX.Element} A `div` containing the loading spinner and a progress message.
 */
...
/**
 * A component to display an error message when the deletion of users fails due to internal error.
 * The component visually indicates an error using a styling class and an icon.
 * It provides a specific message based on the error status code from the deletion result:
...
/**
 * A component to display the results of a user deletion operation.
 *
 ...
 * @prop {number} deletionResult.responseData.deletedUsers - The number of users that were removed successfully.
 * @prop {number} deletionResult.responseData.undeletedUsers - The number of users that were not removed due to errors.
    ...
    <div className="flex flex-col gap-y-6">
      <div className="flex flex-col gap-y-4">
        ...
        <div className={`flex text-base gap-x-2 items-center`}>
          ...
          {(deletionResult.status === userStrings.deletionResultStatuses.failure ||
            deletionResult.responseData.undeletedUsers.length > 0) && (
            <>
          ...
          {deletionResult.status === userStrings.deletionResultStatuses.success &&
            deletionResult.responseData.undeletedUsers.length === 0 && (
              <>
                ...
                <div>{`${deletionResult.responseData.deletedUsers.length} user(s) were deleted successfully`}</div>
              </>
            )}
        </div>
      </div>
...
/**
 * A component to display the results of a user deletion operation in a table format.
 *
 * The component receives the deletion result object as a prop and displays the result
...
function DeletionResultTable({ deletionResult }) {
  ...
  let undeletedUsersArray = []
  const resultTableRef = useRef(null)

  useEffect(() => {
    if (data.deletedUsers.length > 0) {
      deletedUsersArray = data.deletedUsers.map((user) => {
        return {
          email: user.email,
    ...
    if (data.undeletedUsers.length > 0) {
      undeletedUsersArray = data.undeletedUsers.map((user) => {
        let reason
    ...
    setUserList([...deletedUsersArray, ...undeletedUsersArray])
  }, [])
