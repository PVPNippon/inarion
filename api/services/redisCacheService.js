...
const getClient = (redisTransaction) => redisTransaction ?? redisClient
...
const scanKeys = async (req, res, redisTransaction = null) => {
  try {
    ...
    console.log('result.keys.length = ', result.keys.length)
...
const setJsonInRedis = async (key, jsonObject, redisTransaction = null, ttl) => {
  try {
    ...
    console.error(`Error setting JSON in Redis for key "${key}":`, err)
    throw err
  }
}
...
const getJsonFromRedis = async (key, redisTransaction = null) => {
  try {
    ...
    console.log(`Retrieved JSON object for key "${key}":`, jsonObject)
    return jsonObject
  } catch (err) {
    console.error(`Error retrieving JSON object from Redis for key "${key}":`, err)
    throw err
  }
}
...
const getJsonFieldFromRedis = async (key, path, redisTransaction = null) => {
  try {
    ...
    console.error(`Error retrieving JSON field for key "${key}" at path "${path}":`, err)
    throw err
  }
}
...
const updateJsonFieldInRedis = async (key, path, value, redisTransaction = null) => {
  try {
    ...
    console.error(`Error updating JSON field for key "${key}" at path "${path}":`, err)
    throw err
  }
}
...
const deleteJsonFromRedis = async (key, redisTransaction = null) => {
  try {
    ...
    console.error(`Error deleting JSON object for key "${key}":`, err)
    throw err
  }
}
...
const setHashInRedis = async (key, hashObject, redisTransaction = null, ttl) => {
  try {
    ...
    await client.hSet(key, sanitizedObject) // console.log(`Hash set in Redis for key "${key}" with fields:`, sanitizedObject)
  } catch (err) {
    console.error(`Error setting hash in Redis for key "${key}":`, err)
    throw err
  }
}
...
const getHashFromRedis = async (key, redisTransaction = null) => {
  try {
    ...
    console.error(`Error retrieving hash from Redis for key "${key}":`, err)
    throw err
  }
}
...
const getHashFieldFromRedis = async (key, field, redisTransaction = null) => {
  try {
    ...
    console.error(`Error retrieving field "${field}" from hash for key "${key}":`, err)
    throw err
  }
}
...
const hashFieldExists = async (key, field, redisTransaction = null) => {
  try {
    ...
    console.error(`Error checking existence of field "${field}" in hash for key "${key}":`, err)
    throw err
  }
}
...
const deleteHashFieldsFromRedis = async (key, fields, redisTransaction = null) => {
  try {
    ...
    console.error(`Error deleting fields from hash for key "${key}":`, err)
    throw err
  }
}
...
const addToSetInRedis = async (key, member, redisTransaction = null, ttl) => {
  try {
    ...
    console.error(`Error adding members to set "${key}":`, err)
    throw err
  }
}
...
const getSetMembers = async (key, redisTransaction = null) => {
  try {
    ...
    console.log(`Retrieved members of set "${key}":`, members)
    return members
  } catch (err) {
    console.error(`Error retrieving members of set "${key}":`, err)
    throw err
  }
}
...
const isMemberOfSet = async (key, member, redisTransaction = null) => {
  try {
    ...
    console.error(`Error checking membership of value "${member}" in set "${key}":`, err)
    throw err
  }
}
...
const removeFromSet = async (key, members, redisTransaction = null) => {
  try {
    ...
    console.error(`Error removing members from set "${key}":`, err)
    throw err
  }
}
...
const unionOfSets = async (keys, redisTransaction = null) => {
  try {
    ...
    console.error(`Error performing union operation on sets ${keys}:`, err)
    throw err
  }
}
...
const intersectionOfSets = async (keys, redisTransaction = null) => {
  try {
    ...
    console.error(`Error performing intersection operation on sets ${keys}:`, err)
    throw err
  }
}
...
const getSetSize = async (key, redisTransaction = null) => {
  try {
    ...
    console.error(`Error getting size of set "${key}":`, err)
    throw err
  }
}
